...
describe('Customer e2e test', () => {
  ...
  it('should create and save Customers', async () => {
    ...
    await customerComponentsPage.clickOnCreateButton();
    await promise.all([
      customerUpdatePage.setCustNoInput('custNo'),
      customerUpdatePage.setCustContactIdInput('custContactId'),
      customerUpdatePage.setCustContactLastNameInput('custContactLastName'),
      customerUpdatePage.setCustContactFirstNameInput('custContactFirstName')
    ]);
    expect(await customerUpdatePage.getCustNoInput()).to.eq('custNo', 'Expected CustNo value to be equals to custNo');
    expect(await customerUpdatePage.getCustContactIdInput()).to.eq(
      'custContactId',
      'Expected CustContactId value to be equals to custContactId'
    );
    expect(await customerUpdatePage.getCustContactLastNameInput()).to.eq(
      'custContactLastName',
      'Expected CustContactLastName value to be equals to custContactLastName'
    );
    expect(await customerUpdatePage.getCustContactFirstNameInput()).to.eq(
      'custContactFirstName',
      'Expected CustContactFirstName value to be equals to custContactFirstName'
    );
